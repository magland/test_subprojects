...
MemoryMda::MemoryMda(QObject *parent) : QObject(parent)
{
...
MemoryMda::MemoryMda(const MemoryMda &other) : QObject()
{
	d=new MemoryMdaPrivate;
	d->q=this;
...
void MemoryMda::write(const QString &path)
{
	...
	for (int i=0; i<MDAIO_MAX_DIMS; i++) HH.dims[i]=d->m_size[i];
	HH.num_bytes_per_entry=4;
	int num_dims=2;
	for (int i=3; i<MDAIO_MAX_DIMS; i++) {
		if (d->m_size[i]>1) num_dims=i+1;
	}
...
void MemoryMda::allocate(int N1, int N2, int N3, int N4, int N5, int N6)
{
	...
	for (int i=0; i<MDAIO_MAX_DIMS; i++) dims[i]=1;
	dims[0]=N1;
	dims[1]=N2;
	dims[2]=N3;
	dims[3]=N4;
