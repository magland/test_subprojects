...
public:
	...
	CVView *m_view;
	DiskReadMda m_features;
	DiskReadMda m_clips;
	DiskReadMda m_labels;
...
void CVWidget::setFeatures(const DiskReadMda &X)
{
	d->m_features=X;
}

void CVWidget::setClips(const DiskReadMda &X)
{
	d->m_clips=X;
	d->generate_features_from_clips();
}

void CVWidget::setLabels(const DiskReadMda &L)
{
	d->m_labels=L;
}
